...
define gamepoint = 0
image tran = Image("transition.png")
...
image transitionRhythm = Image(IMG_DIR + "transition.png")
...
screen rhythm_game(rhythm_game_displayable,bgImage="transition.png"):
...
init python:
    ...
    def read_notemap_file(notemap_path):
        notemap_path_full = os.path.join(config.gamedir, notemap_path)
        with renpy.file(notemap_path, 'utf-8') as f:
            text = f.read()

        note_dir = [int(string) for string in text.split('\n') if string != '']
        return note_dir
    ...
    class Song():
        def __init__(self, name, audio_path, beatmap_path, notemap_path, beatmap_stride=2):
            ...
            self.notemap = read_notemap_file(notemap_path)
    ...
    class RhythmGameDisplayable(renpy.Displayable):
        ...
        def __init__(self, song):
            ...
            self.track_bar_width = 3
            self.horizontal_bar_height = 4
            ...
            self.random_track_indices = song.notemap #[
            ...
            self.track_bar_drawable = Solid('#64646433', xsize=self.track_bar_width, ysize=self.track_bar_height)
            self.horizontal_bar_drawable = Solid('#ffffffb0', xsize=config.screen_width, ysize=self.horizontal_bar_height)
